...
<script>
	...
	function sav(x) {
		...
		alert(id);
		var td_id=$("#"+id).parent().find("*").eq(0).text();
		alert(td_id);
		var td_keeper=$("#"+id).parent().find("*").eq(1).text();
		var td_location=$("#"+id).parent().find("*").eq(2).text();
		var td_modifiedTime=$("#"+id).parent().find("*").eq(3).text();
		var td_name=$("#"+id).parent().find("*").eq(4).text();
		var td_price=$("#"+id).parent().find("*").eq(5).text();
		var td_quantity=$("#"+id).parent().find("*").eq(6).text();
		var td_type=$("#"+id).parent().find("*").eq(7).text();
		...
		$.ajax({
			...
			data:{"id":td_id,
				...
				"modifiedTime":td_modifiedTime,
				"name":td_name,
				"price":td_price,
				"quantity":td_quantity,
				"type": td_type},
			success : function(m) {
